...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope culture-tools
...
scope:** the default personal scope is **private** (`--scope culture-tools
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `culture-tools`). Private records are served only to a recall in the same
