...
culture-tools vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. culture-tools
...
#   - consumer-identifying prose: `guildmaster` → `culture-tools` (NOT
...
If a re-sync would lose a culture-tools adaptation, lift the change
...
#     → `which culture-tools vendors from guildmaster`
